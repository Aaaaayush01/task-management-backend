...
function seedDemo() {
  ...
  state.projects = [{ id: pid, name: "Tasks", color: "#8B5CF6" }];
...
function renderProjects() {
  ...
  if (!state.projects.length) {
    list.innerHTML =
      '<p style="font-size:0.78rem;color:var(--text-3);padding:0.5rem 0.25rem">No projects yet.</p>';
    return;
  }
...
function updateBreadcrumb() {
  ...
  const el = $("breadcrumbProject");
  if (el) {
    el.textContent = p ? p.name : "Select a project";
  }
}
...
function saveTask() {
  ...
  const priority =
    document.querySelector("#prioritySelector .priority-btn.active")?.dataset
      .priority || "medium";
  const status =
    document.querySelector("#statusSelector .status-btn.active")?.dataset
      .status || "pending";
...
function updateStats() {
  const pid = state.activeProjectId;
  const tasks = pid
    ? state.tasks.filter((t) => t.projectId === pid)
    : state.tasks;
...
function init() {
  ...
  $$("#projectColorPicker .color-swatch").forEach((sw) => {
    sw.addEventListener("click", () => {
      ...
      $$("#projectColorPicker .color-swatch").forEach((s) =>
...
function checkDueDates() {
  ...
  state.tasks
    ...
    .forEach((t) => {
      ...
      if (d <= today) {
        setTimeout(
          () => toast(`"${t.title}" is overdue!`, "error", "⏰"),
          1500,
        );
      } else if (d <= soon) {
